...
##punto b
Let's solve the inverse by using the function solve(A) and let's compute how long it takes throught the function system.time.
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
set.seed(0); n <- 1000
A <- matrix(runif(n*n),n,n); x.true <- runif(n)
...
## punto d
Through the way exposed in point b we get the median absolute error 2.577135e-11, while through the way exposed in point c we get the median absolute error 1.708318e-12, less than the previous one; moreover, this last solving procedure takes less than the other one in terms of computational cost (as the result of system.time shows). Hence we get that the method of solving $$Ax=y$$ which does not involve the computation of the inverse matrix is more efficient.
