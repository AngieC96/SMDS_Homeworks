...
$$
\begin{enumerate}
\item  Simulate $1000$ values, and calculate the proportion of times the chain visits each of
the states. Compare the proportions given by the simulation with the above theoretical
proportions.
\item  Here is code that calculates rolling averages of the proportions over a number of simulations
and plots the result. It uses the function $\mathsf{rollmean()}$ from the $zoo$ package.
\end{enumerate}
$$ 
...
P=matrix(c(0.6, 0.2, 0.4, 0.2, 0.4, 0.3, 0.2, 0.4,0.3 ), 3, 3)
 #the following function simulates N values of a Markov chain sequence, with transition matrix P (see Exercise 12)
...
Repeat this several times. There should be no consistent pattern in the $\mathsf{acf}$ plot for different
random samples $\mathsf{y1}$. There will be a fairly consistent pattern in the $\mathsf{acf}$ plot for $\mathsf{y}$, a result of
...
Consider solving the matrix equation $Ax=y$ for $x$, where $y$ is a known nxn matrix. The formal solution to the problem is $x= A^{-1}y$, but it is possible to solve the equation directly, without actually forming $A^{-1}$. This question explores this direct solution. Read the help file for "solve" before trying it.

a. First create and $A$, $x$ and $y$ satisfying $Ax=y$.
...
b. Using "solve", form the matrix $A^{-1}$ explicitly and then form $x_1= A^{-1}y$ and $x.true$ (the approximate mean absolute 'error' in the solution).
...
a. We create and $A$, $x$ and $y$ satisfying $Ax=y$:
...
c. We use "solve" to directly solve for $x$ without forming $A^{-1}$:
...
d. Through the way exposed in point b we get the median absolute error 2.577135e-11, while through the way exposed in point (c) we get the median absolute error 1.708318e-12, less than the previous one; moreover, this last solving procedure takes less than the other one in terms of computational cost (as the result of `system.time` shows). Hence we get that the method of solving $Ax=y$ which does not involve the computation of the inverse matrix is more efficient.
...
hist(v, breaks= 40, probability = TRUE,  main= bquote(s^2), cex.main=1.5)
curve((n/sigma^2) * dchisq(x * (n/sigma^2), df = n - 1),
      add = TRUE, col="blue", lwd=2, main="N(0,1)")
```

We can see that $s^2_b$ is a biased estimator for the variance because its mean is lower than $1$.

 ```{r, echo=TRUE}
par (mfrow=c(1,1), oma=c(0,0,0,0))
hist(v, breaks= 40, probability = TRUE,  main= "Comparison", cex.main=1.5)
curve((n/sigma^2) * dchisq(x * (n/sigma^2), df = n - 1),
      add = TRUE, col="blue", lwd=2)
curve(((n-1)/sigma^2) * dchisq(x * ((n-1)/sigma^2), df = n - 1),
      add = TRUE, col="red", lwd=2)
legend("topright", legend = c("s^2", "s^2_b"), col = c("red", "blue"), lty = 1)
```
