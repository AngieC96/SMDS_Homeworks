...
The point `Flagstaff to Carling` has now high Cook distance for both models, so we try to remove this point too.

```{r 6.6b4}
nihills.without.flagstaff <- nihills.without.annalong %>% subset(rownames(nihills.without.annalong) != "Flagstaff to Carling")
...
We removed all problematic points. We check the `anova` test again:
...
```{r 6.10b, echo=TRUE}
xyplot(y~x1, groups=x11, data=gas_mil, type = c("p", "r"), pch=20, cex=1.5, xlab="displacement", ylab="mpg", main="Scatterplot")
```
